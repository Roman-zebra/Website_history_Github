...
$('archive').onclick=()=>dialog('archiveDialog');$('help').onclick=()=>dialog('helpDialog');$('pause').onclick=()=>dialog('pauseDialog');
...
window.addEventListener('jta-walk-ready',()=>{
 ...
 for(const id of ['pause','destination','camera','reset','run','mouse','walkYear'])$(id).disabled=false;
 $('camera').textContent=t.first;$('viewStatus').textContent=t.ready;
 function destinations(){
  ...
  destinations();$('walkYear').onchange=()=>{stop();api.setYear(+$('walkYear').value,0);g.reset();$('details').hidden=true;$('camera').disabled=false;$('eraLabel').textContent=$('walkYear').selectedOptions[0].textContent+' · '+t.first;destinations();};
