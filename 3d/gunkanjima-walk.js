...
function enter(id){closeTools();stop();if(g.enter(id)){$('details').hidden=false;$('camera').disabled=true;$('viewStatus').textContent='';}else $('viewStatus').textContent=t.error;}
...
$('reset').onclick=()=>{closeTools();stop();g.reset();$('details').hidden=true;$('camera').disabled=false;};
...
window.addEventListener('jta-walk-ready',()=>{
 ...
 function destinations(){
  ...
  destinations();$('walkYear').onchange=()=>{closeTools();stop();api.setYear(+$('walkYear').value,0);g.reset();$('details').hidden=true;$('camera').disabled=false;$('eraLabel').textContent=$('walkYear').selectedOptions[0].textContent+' · '+t.first;destinations();};
