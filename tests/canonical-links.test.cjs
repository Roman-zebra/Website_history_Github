...
const crawlable=()=>['index.html','explore.html','ja.html','ko.html','zh-cn.html','zh-tw.html','th.html','places.html','visit.html','about.html','supporters.html',...htmlIn('place'),...htmlIn('guides'),...htmlIn('visit')];
...
test('no crawlable page reaches a place through a #hash or a .html duplicate',()=>{
 ...
 for(const file of crawlable()){const s=read(file);
  for(const [,href] of s.matchAll(/href=["']([^"']+)["']/g)){
   ...
   assert.ok(!internal||!/(^|\/)(index|ja|ko|zh-cn|zh-tw|th|places|about|supporters|visit)\.html$/.test(before.split('?')[0]),file+' -> '+href);
  }
