...
test('the about page explains the support in all six languages, says personal details are not shared, and names the hashtags',()=>{
 ...
 for(const id of ['ja','en','ko','zh-Hans','zh-Hant','th']){
  ...
  assert.match(section,/<p class="privacy-note">[^<]+<\/p>/,id+' section does not say that personal details are not shared');}
});
