...
test('locale pages expose crawlable body, reciprocal languages, canonical and valid JSON-LD',()=>{
 for(const [file,l] of [['ja.html','ja'],['ko.html','ko'],['zh-cn.html','zh-Hans'],['zh-tw.html','zh-Hant']]){
  ...
  const schema=JSON.parse(h.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/)[1]);assert.equal(schema.inLanguage,l);assert.equal(schema.mainEntity.numberOfItems,115);const prefix={ja:'ja',ko:'ko','zh-Hans':'zh-cn','zh-Hant':'zh-tw'}[l];assert.ok(h.includes('/place/'+prefix+'/kyoto'));assert.ok(fs.readFileSync(path.join(root,'place',prefix,'kyoto.html'),'utf8').includes('/?lang='+l+'&amp;place=kyoto'));
 }
});
