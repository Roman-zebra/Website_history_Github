...
test('four chapters per round, no consecutive repeat, bounded image cache',async()=>{const h=boot();await h.start();let seen=[h.current()];for(let i=0;i<23;i++){await h.tick(4500);await h.finish();seen.push(h.current());assert.notEqual(seen[i],seen[i+1]);}for(let i=0;i<24;i+=4)assert.equal(new Set(seen.slice(i,i+4)).size,4);assert.equal(h.images.length,10);assert.equal(h.els.heroScene.children.length,4)});
