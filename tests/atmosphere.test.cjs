...
test('four chapters per round, no consecutive repeat, bounded image cache',async()=>{const h=boot();await h.start();let seen=[h.current()];for(let i=0;i<23;i++){await h.tick(8000);await h.finish();seen.push(h.current());assert.notEqual(seen[i],seen[i+1]);}for(let i=0;i<24;i+=4)assert.equal(new Set(seen.slice(i,i+4)).size,4);assert.equal(h.images.length,10);assert.equal(h.els.heroScene.children.length,4)});
...
test('in-flight photo waits for resume without another request',async()=>{const h=boot();await h.start();const before=h.current();await h.tick(8000);h.els.heroMotion.click();await h.finish();assert.equal(h.current(),before);assert.equal(h.timers.size,0);const n=h.images.length;h.els.heroMotion.click();assert.notEqual(h.current(),before);assert.equal(h.images.length,n)});
test('failed photos keep current image and are not requested again',async()=>{const h=boot();await h.start();const before=h.current();await h.tick(8000);const failedSrc=h.images.filter(x=>!x.done).map(x=>x.src);await h.finish(false);assert.equal(h.current(),before);for(let i=0;i<9;i++){await h.tick(8000);await h.finish()}for(const src of failedSrc)assert.equal(h.images.filter(x=>x.src===src).length,1)});
test('localized captions and explicit photo source/license links',async()=>{const h=boot();await h.start();for(let i=0;i<4;i++){const ja=h.els.heroCaption.textContent;h.document.documentElement.lang='en';h.observers[0]();assert.notEqual(h.els.heroCaption.textContent,ja);assert.match(h.els.heroCredit.href,/^https:/);if(/landmark|liminal/.test(h.current())){assert.equal(h.els.heroLicense.hidden,false);assert.match(h.els.heroLicense.href,/creativecommons.org/)}h.document.documentElement.lang='ja';h.observers[0]();await h.tick(8000);await h.finish()}});
