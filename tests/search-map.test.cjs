...
function runtime(){
 ...
 const ctx={map,devicePixelRatio:1,LANG:'ja',qSeq:0,JapanBoundary:require('../japan-boundary.js'),PlaceUI:require('../place-ui.js'),document:{getElementById:id=>els.get(id)},$:id=>{if(!els.has(id))els.set(id,element());return els.get(id);},closePanel:()=>state.closed++,showSavedSpot:r=>state.opened.push(r),openMonument:id=>state.opened.push(id),noPush:fn=>fn(),openMap(){},L:{Layer:{extend:o=>class{constructor(){Object.assign(this,o);}addTo(m){this.onAdd(m);return this;}}},DomUtil:{create:()=>canvas,setPosition(){}},latLngBounds:r=>r}};
