...
## 歩いて探索・天気と時刻（ビューア v7、2026-09-26）

データは増やさず、`gunkanjima-3d.js` だけで動く。検証は `tests/lab-3d-walk.test.cjs`。

- 「🚶 歩いて探索」で一人称になる。目の高さは地面から1.55m。地面は描いている地形そのもの（地形の格子は写真の座標なので、TERRAIN_VS の高さによるずれを逆に解いて足もとの高さを出す）。体は直径0.6m。護岸の内側から出られず、その年に建っている建物の壁で止まる。足もとから0.4m以内の屋根には上がれる。
- 室内（`gunkanjima-interiors.json` の12か所）では床・階段・回廊に立ち、家具で止まる。戸・カーテン・洗濯物・小物は通り抜ける（`PASS`）。入ると、その場面のカメラが床から立った高さにあればそこに、そうでなければカメラが見ている所に立つ（地下の共同浴場は上の地面ではなく浴場の床）。
- 操作：W A S D か矢印キー、ドラッグで見回す、Shift で走る、地面をクリックでそこまで歩く、建物をクリックで説明。スマホは左下を親指で押すとスティック。右上の島の地図をタップするとその場所へ。正面の建物の名前・竣工年・用途が画面下に出る。建物の後ろに隠れた名前とピンは出さない。
- 天気（模型の見た目・晴れ・くもり・雨・海霧・台風）と時刻。太陽は、表示中の写真の撮影日のその時刻（日本時間）の位置（北緯32.6278°・東経129.7386°で計算。1962年5月30日10時が従来の光の向きと4°以内）。「模型の見た目」は従来どおりの暗い背景と照明で、既定。夜は1974年までの年だけ住宅の窓と室内に明かりが灯る。
- URL：`?walk=1`（歩いて始める）、`?sky=rain`、`?hour=19`（`scene=` と組み合わせ可）。
- 壁のベランダがざらついて見えたのは、頂点ごとに同じはずの seed が補間で画素ごとにわずかにずれ、hash() で拡大されていたため。89段に丸めて直した。
...
- 屋根と地面は空中写真そのものなので、時刻を変えても写真に写った影（午前10時ごろ）は残ります。歩いて近づくと地面は写真の解像度（1画素約0.4m）でぼやけるため、近くだけ細かな模様を重ねています。
